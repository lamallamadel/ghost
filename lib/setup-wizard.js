...
class SetupWizard {
    ...
    async promptNLRouter() {
        console.log(`${Colors.BOLD}Natural Language Router${Colors.ENDC}`);
        console.log(`${Colors.DIM}Enables semantic command matching in the Ghost interactive shell${Colors.ENDC}`);
        console.log(`${Colors.DIM}Uses a local 22M-parameter model (~25MB, downloaded once to ~/.ghost/models/)${Colors.ENDC}`);
        console.log(`${Colors.DIM}Latency: ~5–15ms per query on CPU. No network at runtime.${Colors.ENDC}`);
        const answer = await this.prompt(`Enable semantic NL routing? (y/n) [n]: `);
        if (answer.trim().toLowerCase() === 'y') {
            this.config.nlRouter.mode = 'semantic';
            console.log(`${Colors.GREEN}✓${Colors.ENDC} Semantic routing enabled — model downloads on first shell start`);
        } else {
            console.log(`${Colors.DIM}Using keyword routing (default)${Colors.ENDC}`);
        }
        console.log('');
    }
